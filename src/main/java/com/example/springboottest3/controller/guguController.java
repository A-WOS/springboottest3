...
public class guguController {
    ...
    public String guguf(@RequestParam(name="key", required = false, defaultValue = "World") String key, Model model) {
        model.addAttribute("key", key);
        return "guguf"; // html name

    }
    @GetMapping("/backend/gugu") // gugu라는 함수의 mapping 경로는 /backend/gugu가 됨.
    public String gugub(@RequestParam(name="key", required = false, defaultValue = "World") String key, Model model) {
        model.addAttribute("key", key);
